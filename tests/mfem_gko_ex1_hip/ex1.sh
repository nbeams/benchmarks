...
function run_tests()
{
   ...
   # 'ceed' can be set on the command line
   local l_ceed=(${ceed:-/cpu/self/ref/serial})
   ...
   for dev in ${l_mfem_devs[@]}; do
      for ((p = l_min_p; p <= l_max_p; p++)) do
         for ((l = 0; (p**dim)*(2**l) <= l_max_dofs_proc; l++)) do
            args=(-p $problem -o $p -l $l -d $dev -c $l_ceed -e $el_type)
            if [ -z "$dry_run" ]; then
               echo "Running test:"
...
packages="hip mfem-serial-no-ceed"
